...
const CreatePost = () => {
  ...
  const handleSubmit = async ({ title, body }: { title?: string; body: string }) => {
    ...
    if (!user) {
      toast.error(t("create.toast.login"));
      return;
    }
    if (!title || !targetCategory) {
      toast.error(t("create.toast.categoryTitle"));
      return;
    }
    try {
      setSubmitting(true);
      ...
      toast.error(error instanceof Error ? error.message : t("create.toast.error"));
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <PageTransition>
      <div className="space-y-6">
        <header className="rounded-3xl border border-border/60 bg-card/70 p-6">
          <h1 className="text-2xl font-bold">{t("create.header")}</h1>
          <p className="text-sm text-muted-foreground">{t("create.subheader")}</p>
        </header>
        <div className="space-y-4 rounded-3xl border border-border/60 bg-card/70 p-6">
          <div className="space-y-2">
            <span className="text-sm font-medium text-muted-foreground">{t("create.category")}</span>
            {isAdmin ? (
              <div className="relative">
                ...
                >
                  <option value="">{t("create.category.select")}</option>
              ...
              <div className="rounded-xl border border-border/60 bg-background/60 px-4 py-3 text-sm">
                {activeCategory ? activeCategory.name : t("create.category.loading")}
              </div>
            )}
          </div>
